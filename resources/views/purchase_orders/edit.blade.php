...
@section('content')
    ...
    <div class="max-w-2xl">
        <div class="mb-3">
            <h1 class="text-xl font-semibold">Ubah Purchase Order</h1>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white px-5 pt-2 pb-4">
            <form method="POST" action="{{ route('purchase-orders.update', $po) }}" enctype="multipart/form-data" class="space-y-3">
                ...
                <div>
                    ...
                    <input name="nomor_po" value="{{ old('nomor_po', $po->nomor_po) }}"
                ...
                <div>
                    ...
                    <input name="judul" value="{{ old('judul', $po->judul) }}" required
                        class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10">
                ...
                @if($pelangganLuar)
                    <div>
                        ...
                        <input name="pembeli_nama" value="{{ old('pembeli_nama', $po->pembeli_nama) }}" required
                            class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10">
                ...
                @else
                    ...
                    <div>
                        ...
                        <textarea name="supplier_alamat" rows="3" {{ $aturSendiri ? '' : 'disabled' }}
                            class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10 disabled:bg-slate-50 disabled:text-slate-500"
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="date" name="tgl_po" value="{{ old('tgl_po', $po->tgl_po?->format('Y-m-d')) }}" required
                            class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10">
                ...
                <div>
                    ...
                    <select name="jenis_transaksi" {{ $dariUsulan ? 'disabled' : 'required' }}
                ...
                <div>
                    ...
                    <input type="number" min="1" name="total" value="{{ old('total', (int) round((float) $po->total)) }}" required
                        class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10">
                ...
                <div>
                    ...
                    <textarea name="catatan" rows="3"
                        class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10"
                        placeholder="Catatan tambahan (opsional)">{{ old('catatan', $po->catatan) }}</textarea>
                ...
                <div class="flex flex-col-reverse gap-3 border-t border-slate-100 pt-4 sm:flex-row sm:justify-end">
                    ...
                    <button type="submit"
                        ...
                        {{ $dikirimUlang ? 'Simpan & Kirim Ulang' : 'Simpan Perubahan' }}
                    </button>
                </div>
            </form>
        </div>
