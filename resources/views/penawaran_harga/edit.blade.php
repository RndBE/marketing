...
@section('content')
    <div class="w-full max-w-4xl">
        ...
        <form method="POST" action="{{ route('penawaran-harga.update', $usulan->id) }}" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <select name="target_company_id" required class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
                            @foreach($companies as $company)
                                <option value="{{ $company->id }}" {{ (int) old('target_company_id', $usulan->target_company_id) === (int) $company->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="jenis_transaksi" required class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
                            @php
                                $jenis = old('jenis_transaksi', $usulan->jenis_transaksi);
                            @endphp
            ...
            @include('penawaran_harga.partials.signature-fields', ['usulan' => $usulan])
                ...
                <div id="item-container">
                    ...
                    @if (is_array($oldJudul))
                        @foreach ($oldJudul as $i => $judul)
                            @include('penawaran_harga.partials.item-row', [
                                'index' => $i,
                                'judul' => $judul,
                    ...
                    @elseif ($usulan->items->count())
                        @foreach ($usulan->items as $i => $item)
                            @include('penawaran_harga.partials.item-row', [
                                ...
                                'satuan' => $item->satuan,
                                'harga' => $item->harga ?? 0,
                                'tipe' => $item->tipe ?? 'custom',
                                'productId' => $item->product_id,
                                'points' => preg_split('/\R/', (string) $item->catatan, -1, PREG_SPLIT_NO_EMPTY),
                            ])
                        @endforeach
                    @else
                        @include('penawaran_harga.partials.item-row', [
                        ...
                        @foreach ($usulan->attachments as $att)
                            <div class="flex flex-col gap-2 rounded-lg bg-slate-50 px-3 py-2 sm:flex-row sm:items-center sm:justify-between">
                                <div class="min-w-0">
                                    ...
                                    <span class="break-all text-sm">{{ $att->nama_file }}</span>
                                </div>
                                <div class="flex items-center gap-3">
                                    <a href="{{ route('penawaran-harga.attachments.download', [$usulan, $att]) }}"
            ...
            <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                <a href="{{ route('penawaran-harga.show', $usulan->id) }}"
