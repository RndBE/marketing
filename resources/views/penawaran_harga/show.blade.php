    ...
    @php
        ...
        if ($isRequester) {
            ...
            if (! $usulan->penawaran_id && in_array($usulan->status, ['menunggu', 'ditanggapi', 'disetujui'], true)) {
                ...
                $nextActionUrl = route('penawaran-harga.quotation.show', $usulan);
    ...
    <div class="w-full max-w-5xl">
        <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('penawaran-harga.pdf', $usulan) }}" target="_blank"
        ...
        <div id="tanda-tangan-permohonan" class="mb-4 scroll-mt-4 rounded-xl border border-slate-200 bg-white p-5">
            ...
            @if($canEditRequestSignature)
                <form method="POST" action="{{ route('penawaran-harga.signature.update', $usulan) }}" enctype="multipart/form-data" class="space-y-3">
                    @csrf
                ...
                @if($usulan->signature_path)
                    <form method="POST" action="{{ route('penawaran-harga.signature.delete', $usulan) }}" class="mt-2"
                ...
                <div class="space-y-2">
                    @foreach ($usulan->attachments as $att)
                        <div class="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2">
                            ...
                            <a href="{{ route('penawaran-harga.attachments.download', [$usulan, $att]) }}" class="text-sm font-semibold text-blue-600">Download</a>
                        </div>
                    @endforeach
                </div>
            </div>
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @if ($canViewLinkedPenawaran)
                            <a href="{{ route('penawaran-harga.quotation.show', $usulan) }}" class="rounded-xl border border-emerald-300 bg-white px-4 py-2.5 text-sm font-semibold text-emerald-700">Lihat Penawaran Harga</a>
                        ...
                        @if($canEditLinkedQuotation)
                            <a href="{{ route('penawaran-harga.quotation.show', $usulan) }}#tanda-tangan-penawaran"
        ...
        @if($isRequester && $usulan->penawaran_status === 'sent')
            <form id="keputusan-penawaran" method="POST" action="{{ route('penawaran-harga.tanggapi-penawaran', $usulan) }}" class="mb-4 scroll-mt-4 rounded-xl border border-blue-200 bg-white p-5">
                @csrf
        ...
        <div id="aksi-transaksi" class="mb-4 flex scroll-mt-4 flex-wrap gap-2">
            ...
            @if ($canRespond && $usulan->status === 'disetujui' && !$usulan->penawaran_id)
                <form action="{{ route('penawaran-harga.buat-penawaran', $usulan) }}" method="POST">
                    @csrf
